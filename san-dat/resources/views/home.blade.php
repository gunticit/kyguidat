...
@section('content')
    ...
    <script>
        ...
        function renderPropertyCard(item) {
            ...
            let gridItems = '';
            const gi = 'style="margin:0;padding:1px 0;"';
            if (item.area_dimensions) gridItems += `<p ${gi}><span style="color:#9ca3af;">Diện tích:</span> ${item.area_dimensions}</p>`;
            if (item.residential_area) gridItems += `<p ${gi}><span style="color:#9ca3af;">Thổ cư:</span> ${cleanNumber(item.residential_area)} m²</p>`;
            if (dirText) gridItems += `<p ${gi}><span style="color:#9ca3af;">Hướng:</span> ${dirText}</p>`;
            if (item.road) gridItems += `<p ${gi}><span style="color:#9ca3af;">Loại đường:</span> ${item.road}</p>`;
            if (item.frontage_actual && item.frontage_actual !== '0' && item.frontage_actual !== '0.00') gridItems += `<p ${gi}><span style="color:#9ca3af;">Mặt tiền:</span> ${parseFloat(item.frontage_actual)} m</p>`;
            if (item.has_house) gridItems += `<p ${gi}><span style="color:#9ca3af;">Tình trạng:</span> ${item.has_house === 'co' || item.has_house === 'yes' ? 'Có nhà' : 'Chưa bán'}</p>`;
            gridItems += `<p ${gi}><span style="color:#f97316;font-weight:bold;">Giá: ${price}</span></p>`;
