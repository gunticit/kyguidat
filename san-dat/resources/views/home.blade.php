...
@section('content')
                ...
                <div id="advancedFilters" class="hidden border-t border-navy-600 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div class="lg:col-span-2">
                            ...
                            <input type="text" name="phone" placeholder="Nhập số thứ tự hoặc số điện thoại"
                                class="w-full px-3 py-2 bg-navy-700 border border-navy-600 rounded-lg focus:border-green-500 focus:outline-none text-gray-200 placeholder-gray-500">
                        </div>
    ...
    <section class="py-16 bg-navy-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold text-gray-100 mb-8">Bất động sản cần bán</h2>
                ...
                <div>
                    <div id="locationStatus" class="flex items-center gap-2 text-sm">
                        <button onclick="requestLocation()"
                            class="inline-flex items-center text-gray-400 hover:text-green-400 transition cursor-pointer bg-transparent border-0 p-0">
    ...
    <script>
        ...
        function updateLocationStatus(status) {
            ...
            if (status === 'active') {
                ...
                el.innerHTML = `
                                                <button onclick="requestLocation()" class="inline-flex items-center text-gray-400 hover:text-green-400 transition cursor-pointer bg-transparent border-0 p-0">
                                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        function renderPropertyCard(item) {
            ...
            const typeLabel = (Array.isArray(landTypes) && landTypes.length > 0) ? landTypes[0] : (item.type || '');
            ...
            let addressHtml = '';
            if (item.address) addressHtml = `<p class="mb-1"><span class="text-gray-500">Địa chỉ:</span> ${item.address}</p>`;
            let gridItems = '';
            if (item.area_dimensions) gridItems += `<p><span class="text-gray-500">Diện tích:</span> ${item.area_dimensions}</p>`;
            if (item.residential_area) gridItems += `<p><span class="text-gray-500">Thổ cư:</span> ${item.residential_area} m²</p>`;
            if (dirText) gridItems += `<p><span class="text-gray-500">Hướng:</span> ${dirText}</p>`;
            if (item.road) gridItems += `<p><span class="text-gray-500">Loại đường:</span> ${item.road}</p>`;
            if (item.has_house) gridItems += `<p><span class="text-gray-500">Tình trạng:</span> ${item.has_house === 'yes' ? 'Có nhà' : 'Đất trống'}</p>`;

            const distanceBadge = (item.distance !== undefined && item.distance !== null && locationReady)
                ? `<span class="absolute top-2 left-2 px-2 py-1 bg-blue-600/90 text-white text-xs rounded-full font-medium">📍 ${formatDistance(item.distance)}</span>`
                : '';

            return `
                                        <a href="/bat-dong-san/${slug}" class="block bg-navy-700 rounded-lg shadow-md overflow-hidden hover:shadow-xl hover:shadow-green-500/10 transition group border border-navy-600">
                                            <div class="aspect-video bg-navy-800 relative overflow-hidden">
                                                ${imgHtml}
                                                ${typeLabel ? `<span class="absolute top-2 right-2 px-2 py-1 bg-green-500 text-white text-xs rounded-full font-medium">${typeLabel}</span>` : ''}
                                                ${distanceBadge}
                                            </div>
                                            <div class="p-4">
                                                ...
                                                <p class="text-green-400 font-bold text-lg mt-2">${price}</p>
                                            </div>
                                        </a>
                                    `;
        }
        ...
        function renderPropertyListCard(item) {
            ...
            if (item.residential_area) details += `<span><span class="text-gray-500">Thổ cư:</span> ${item.residential_area} m²</span>`;
            ...
            const distanceBadge = (item.distance !== undefined && item.distance !== null && locationReady)
                ? `<span class="absolute bottom-2 left-2 px-2 py-1 bg-blue-600/90 text-white text-xs rounded-full font-medium">📍 ${formatDistance(item.distance)}</span>`
                : '';
