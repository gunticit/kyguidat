...
@section('content')
    ...
    <script>
        ...
        function renderPropertyCard(item) {
            ...
            const dirText = Array.isArray(dirs) && dirs.length > 0 ? dirs.join(', ') : '';
            ...
            if (item.area_dimensions) details += `<p><span class="text-gray-500">Diện tích:</span> ${item.area_dimensions}</p>`;
            if (item.residential_area) details += `<p><span class="text-gray-500">Thổ cư:</span> ${item.residential_area} m²</p>`;
            if (dirText) details += `<p><span class="text-gray-500">Hướng:</span> ${dirText}</p>`;
            if (item.road) details += `<p><span class="text-gray-500">Loại đường:</span> ${item.road}</p>`;
            if (item.has_house) details += `<p><span class="text-gray-500">Tình trạng:</span> ${item.has_house === 'yes' ? 'Có nhà' : 'Đất trống'}</p>`;
