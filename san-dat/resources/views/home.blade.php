...
@section('content')
    ...
    @if(!empty($featuredProvinces) && count($featuredProvinces) > 0)
        <section class="py-12 sm:py-8 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mb-6">Bất động sản theo địa điểm</h2>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4" style="grid-auto-rows: 180px;">
                    @foreach($featuredProvinces as $index => $province)
                        <a href="{{ url('/search?province=' . urlencode($province['name'])) }}"
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-3 md:p-4 z-10">
                                ...
                                <p class="text-gray-300 text-xs md:text-sm mt-0.5">
                                    {{ number_format($province['consignment_count']) }} tin đăng</p>
                            </div>
                        </a>
                    @endforeach
                </div>
    ...
    <!-- Map Section -->
    <section class="py-16 sm:py-8 bg-navy-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <section class="py-16 sm:py-8" id="all-properties-section">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <script>
        ...
        function renderPropertyCard(item) {
            ...
            const distanceBadge = (item.distance !== undefined && item.distance !== null && locationReady)
                ? `<span class="absolute top-2 left-2 px-2 py-0.5 bg-blue-600/90 text-white text-xs rounded-full font-medium">📍 ${formatDistance(item.distance)}</span>`
                : '';

            let gridItems = '';
            if (item.area_dimensions) gridItems += `<p><span class="text-gray-400">Diện tích:</span> ${item.area_dimensions}</p>`;
            if (item.residential_area) gridItems += `<p><span class="text-gray-400">Thổ cư:</span> ${item.residential_area} m²</p>`;
            ...
            return `
                                                    <a href="/bat-dong-san/${slug}" class="flex flex-col md:flex-row bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition group">
    ...
    <!-- CTA Section -->
    <section class="py-16 sm:py-8 bg-gradient-to-r from-green-600 to-green-700">
