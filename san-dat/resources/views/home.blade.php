...
@section('content')
                ...
                <div id="advancedFilters" class="hidden border-t border-navy-600 pt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <select name="property_type"
                                ...
                                <option value="dat_tai_dinh_cu">Đất tái định cư</option>
                                <option value="dat_sao">Đất sào</option>
                                <option value="dat_ray">Đất rẫy</option>
                                <option value="bat_dong_san_nghi_duong">Bất động sản nghỉ dưỡng</option>
                                <option value="dat_phan_lo_du_an">Đất phân lô dự án</option>
                                <option value="chung_cu">Chung cư</option>
                                <option value="dang_su_dung_kinh_doanh">Đang sử dụng kinh doanh</option>
                                <option value="khac">Khác</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-300 mb-1">Nhà trên đất</label>
                            <select name="house_on_land" id="houseOnLandSelect"
                                class="w-full px-3 py-2 bg-navy-700 border border-navy-600 rounded-lg focus:border-green-500 focus:outline-none text-gray-200"
                                onchange="toggleFloorArea()">
                                <option value="">-- Tất cả --</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        <div>
                            ...
                            <select name="floor_area_range" id="floorAreaSelect" disabled
                                class="w-full px-3 py-2 bg-navy-700 border border-navy-600 rounded-lg focus:border-green-500 focus:outline-none text-gray-200 disabled:opacity-50 disabled:cursor-not-allowed">
                                <option value="">-- Tất cả --</option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <select name="area_range"
                                class="w-full px-3 py-2 bg-navy-700 border border-navy-600 rounded-lg focus:border-green-500 focus:outline-none text-gray-200">
                                <option value="">-- Tất cả --</option>
                                <option value="duoi_100">Dưới 100 m²</option>
                                <option value="100_200">Từ 100 - 200 m²</option>
    ...
    <script>
        ...
        function toggleFloorArea() {
            const houseSelect = document.getElementById('houseOnLandSelect');
            const floorSelect = document.getElementById('floorAreaSelect');
            if (houseSelect.value === 'co') {
    ...
    <section class="sm:py-8 py-4" id="all-properties-section">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div id="allPropertiesSkeleton" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @for($i = 0; $i < 12; $i++) @include('components.skeleton-card') @endfor
            </div>
    ...
    <script>
        ...
        function renderPropertyCard(item) {
            ...
            let gridItems = '';
            if (item.area_dimensions) gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Diện tích:</span> ' + item.area_dimensions + '</p>';
            if (item.residential_area) gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Thổ cư:</span> ' + cleanNumber(item.residential_area) + ' m²</p>';
            if (dirText) gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Hướng:</span> ' + dirText + '</p>';
            if (item.road) gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Loại đường:</span> ' + item.road + '</p>';
            if (item.frontage_actual && item.frontage_actual !== '0' && item.frontage_actual !== '0.00') gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Mặt tiền:</span> ' + parseFloat(item.frontage_actual) + ' m</p>';
            if (item.statusText) gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#9ca3af">Tình trạng:</span> ' + item.statusText + '</p>';
            gridItems += '<p style="margin:0;padding:1px 0"><span style="color:#f97316;font-weight:bold">Giá: ' + price + '</span></p>';
    ...
    <script>
        ...
        function buildPopupHTML(property) {
            ...
            if (property.residential_area) popupDetails += '<p style="color:#94a3b8;font-size:12px;margin:0 0 2px"><span style="color:#6b7280">Thổ cư:</span> ' + parseFloat(property.residential_area) + ' m²</p>';
            if (popupDirText) popupDetails += '<p style="color:#94a3b8;font-size:12px;margin:0 0 2px"><span style="color:#6b7280">Hướng:</span> ' + popupDirText + '</p>';
            if (property.road) popupDetails += '<p style="color:#94a3b8;font-size:12px;margin:0 0 2px"><span style="color:#6b7280">Loại đường:</span> ' + property.road + '</p>';
